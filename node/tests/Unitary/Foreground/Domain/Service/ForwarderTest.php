...
class ForwarderTest extends TestCase
{
     ...
     */
    public function testForwardSuccess(Item $item, PreferenceEntry $preferenceEntry, array $nodes, string $expectedSlotCoordinatorId, array $forwardCalls, bool $handleWriteLocally): void
    {
        ...
        $this->localCoordinator
            ->expects(self::exactly($handleWriteLocally ? 1 : 0))
            ->method('handleWriteLocally')
            ->with($item)
            ->willReturn($this->createNode($expectedSlotCoordinatorId));
        ...
        $slotCoordinator = $this->forwarder->forwardWrite($item);
    ...
    public function testForwardFailure(Item $item, PreferenceEntry $preferenceEntry, array $nodes, array $forwardCalls): void
    {
        ...
        $this->forwarder->forwardWrite($item);
    }
